...
public:
	...
	ILDataset(const std::string& _datasetpath)
	{
		...
		if (fs::exists(i1)) indexpath = i1;
		else if (fs::exists(i2)) indexpath = i2;
		else if (fs::exists(i3)) indexpath = i3;
		else if (fs::exists(i4)) indexpath = i4;
		else {
	...
	static bool isdatabase(const std::string& path)
	{
		...
		bool isvalid = fs::exists(indexfile);
		if (isvalid)return true;

		indexfile = p + "index.pd";
		isvalid = fs::exists(indexfile);
		if (isvalid)return true;
		else return false;
	}
